...
const sections = [
   {
	title: `${htjava} OWNER –––––––––·•`,
	rows: [
	{title: "❒ • Kontak Owner", rowId: ".owner kontak"},
...
await conn.sendMessage(m.chat, { contacts: { displayName: wm, contacts: [{ vcard }] }}, { quoted: ftroli })
 ...
 conn.send2ButtonImg(m.chat, fs.readFileSync('./media/xianzhe3.jpg').buffer, teksnomor, `Halo ${tag}\nitu Owner ku\n` + botdate, `Sewa Bot`, `.sewa`, 'Menu', `.menu`, ftroli, { contextInfo: { externalAdReply: { showAdAttribution: true,
   ...
   conn.sendButton(m.chat, '©Xianzhe ID', teksbio, ppown, [
                ['Sewa Bot', `${usedPrefix}sewa`],
                ['Menu', `${usedPrefix}menu`]
            ], m)
            break
